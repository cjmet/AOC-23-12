...
namespace AOC_23_12
{
    ...
    public class Springs
    {
        ...
        public long FindRecursiveMatches(CancellationToken ct)
        {
            return FindRecursiveMatches(0, 0, ct);
        }

        public long FindRecursiveMatches(int springIndex, int clusterIndex, in CancellationToken ct)
        {
            ...
            // Increment Down
            if (SpringsMatchesCluster(springIndex, _data[clusterIndex])) found += FindRecursiveMatches(springIndex + _data[clusterIndex] + 1, clusterIndex + 1, ct);
            // Increment Right
            if (IsSpring(springIndex)) found += FindRecursiveMatches(springIndex + 1, clusterIndex, ct);
